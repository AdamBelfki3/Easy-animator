 ...
 * SVG file format. This view only produces output in text with three shapes:
 * the rectangle, ellipse and also the plus sign shape and then can be viewed using a browser
...
public class SVGAnimationPlus extends SVGAnimationView implements ISVGAnimationView {
  ...
  /**
   * Displays the model in the svg file format but outputs in text format.
      ...
      switch (sm.getShape().shapeName()) {
        case "plus":
          ...
          String openTagPlus2 = String.format("<rect id=\"%sHG\" x=\"%.0f\" y=\"%.0f\" " +
              ...
              sm.getShape().getPos().getX(),
              sm.getShape().getPos().getY() +
                ((double)(sm.getShape().getSize().getHeight()) * 1 / 3),
              sm.getShape().getSize().getWidth(),
              (int)((double)(sm.getShape().getSize().getHeight()) * 1 / 3),
              sm.getShape().getRgb().getR(), sm.getShape().getRgb().getG(),
              sm.getShape().getRgb().getB());
          ...
          for (IMotion m : sm.getMotions()) {
            ...
            String yAttribute =
                String.format("<animate attributeType=\"xml\" begin=\"%dms\" dur=\"%dms\" "
                    ...
                    m.getEndPos().getY() +
                      ((double) (m.getEndSize().getHeight()) * 1 / 3) -
                      this.viewModel.getYCanvas());
            result.append(yAttribute);
