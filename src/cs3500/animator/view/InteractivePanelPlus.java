...
/**
 * This panel contains all the features of the interactive panel except that now it has some more
 * features such as the ability to display shapes in just their outline and display them
 * in discrete or continuous manner.
 */
...
public class InteractivePanelPlus extends InteractivePanel implements IInteractivePanel {
  ...
  /**
   * Since the panel extends the interactive panel, the interactive panel fields
   * are stored as a super.
    ...
    for (String name : allSM.keySet()) {
      ...
      switch (shape.shapeName()) {
        ...
        case "plus":
          if ((sm.getStartTime() <= this.currentTick) && (sm.getEndTime() >= this.currentTick)) {
            if (!this.isOutline) {
              Rectangle2D rectV = new Rectangle2D.Double(shape.getPos().getX()
                  - (double) this.cornerX + ((double) (shape.getSize().getWidth()) / 3),
                  ((shape.getPos().getY() - (double) this.cornerY)),
