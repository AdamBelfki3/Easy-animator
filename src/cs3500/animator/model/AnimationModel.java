...
public interface AnimationModel {
  ...
  /**
   * Displays the shape and motion combined to represent them together and connect them to the
   * corresponding shape depending on the string inputted.
   *
  ...
  /**
   * Begins the animation and then pushes the final drawings to the stack and modifies the fields in
   * simple animation accordingly. First of all, ensures that the drawings in stack motions are
   * valid (ie. not empty, coherent, there are no gaps, there are no overlaps) and then adds them to
   * the stack motions after it's been validate (throws an exception if invalid) Next, sets the
   * other simple animation fields to default values and then starts the game.
   */
  ...
  /**
   * Indicates when the animation should be over which is when the animation model not started. is
   * greater than t max time.
   *
  ...
  /**
   * Retrieves the last saved animation from the drawings stack. This is a feature that can be used
   * if we would like to undo an animation. Also throws an illegal state exception if the drawings
   * are empty since there is no motion that can be retrieved.
   */
  ...
  /**
   * Adds the tempo that is passed in by the user (tempo 1 and tempo 2) and then uses
   * the passed in input to adjust the slow motion of animation accordingly.
   */
  public void addTempo(int t1, int t2);

  /**
   * Determines whether the passed in tick is meant to be in slow motion or not.
   */
  public int isSlowMoTick(int tick);

  /**
   * Stores all the integers of the start and end times which is used in order to render the
   * discrete frames of the animation. We use this list of integers so that we can render the
   * animation in these times.
   */
